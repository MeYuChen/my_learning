...
try:
    ...
    pedestrian = world.get_blueprint_library().find("walker.pedestrian.0029")#乘用车 
    blueprint = []
    ...
    while x<20:
        if x != 0:
            y1 = math.sqrt(400/(x*x))
        y2 = -y1
        x+=1
        print(x,y1)
        print(x,y2)
        spawn_point = carla.Transform (carla.Location(x,y1,0.5),carla.Rotation (0,0,0))
        spawn_points.append(spawn_point) 
        spawn_point = carla.Transform (carla.Location(x,y2,0.5),carla.Rotation (0,0,0))
        spawn_points.append(spawn_point)
    destory_list = []
    for i in spawn_points:
        temp = world.try_spawn_actor(pedestrian,i)
        if temp is not None:
            destory_list.append(temp)
            locations.append(temp.get_location())
    player_control = carla.WalkerControl()
    
    heading =0
    cnt  =0
    while True:
        player_control.speed = random.randrange(1,30,3)
        for i,pedes in enumerate( destory_list):
            if cnt>10:
                cnt = 0
                heading = random.randrange(-180,360,180)
